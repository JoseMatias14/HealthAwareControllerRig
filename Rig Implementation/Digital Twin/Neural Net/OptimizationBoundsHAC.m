function [lbx,lbu,lbtheta,ubx,ubu,ubtheta] = OptimizationBoundsHAC(par)
...
res_theta_ub= 1.*(ones(n_w,1));

%Riser valve caracteristics
top_theta_lb=0.*ones(n_w,1);
top_theta_ub=1.*ones(n_w,1);

%% 
lbx = vertcat(m_o_lb,w_ro_lb,w_pr_lb,p_rh_lb,p_bh_lb,dP_lb);
lbu = vertcat(vo_lb);
lbtheta = vertcat(res_theta_lb,top_theta_lb);

ubx = vertcat(m_o_ub,w_ro_ub,w_pr_ub,p_rh_ub,p_bh_ub,dP_ub);
ubu = vertcat(vo_ub);
