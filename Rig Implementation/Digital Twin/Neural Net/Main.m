% Testing the implementation of the HAC - the code is not representative of
% a real situation is just a mock up to test the implementation of
% LABVIEWMain.m
...
% loading rig data of the DP - dummy
dp_data = load('ExperimentsNN'); 
...
parPlant.nMeas = 6;
parPlant.H = zeros(parPlant.nMeas,24);
...
for kk = 1:nFinal
    ...
    if kk > BufferLength && rem(kk,nExec) == 0
        ...
        I_vector = [measPlantArray(1,(kk - BufferLength + 2):kk + 1); % FI-101 [L/min]
                    ...
                    dp_data.data{2, 1}.temperature((kk - BufferLength + 1):kk)'; % (TI101, TI102, TI103)
                    dp_data.data{2, 2}.temperature((kk - BufferLength + 1):kk)';
                    dp_data.data{2, 3}.temperature((kk - BufferLength + 1):kk)';
                    dp_data.data{2, 1}.deltaP((kk - BufferLength + 1):kk)'; %(dPI101, dPI102, dPI103)
                    dp_data.data{2, 2}.deltaP((kk - BufferLength + 1):kk)'; 
                    dp_data.data{2, 3}.deltaP((kk - BufferLength + 1):kk)'; 
...
save(name,'tgrid','uPlantArray','measPlantArray','flagArray','ofArray','thetaHatArray','yEstArray','yOptArray','uOptArray','uImpArray','compuTimeArray');
