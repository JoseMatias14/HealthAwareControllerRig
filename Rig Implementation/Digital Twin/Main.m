...
parPlant.nMeas = 9;
parPlant.H = zeros(parPlant.nMeas,length(zPlant0));
...
for kk = 1:nFinal/parPlant.T
    ...
    if kk > BufferLength && rem(kk,nExec) == 0
        ...
        I_vector = [measPlantArray(1,(kk - BufferLength + 2):kk + 1); % FI-101 [L/min]
                    ...
                    (measPlantArray(7,(kk - BufferLength + 2):kk + 1)); % dPI-101 [mbar]
                    (measPlantArray(8,(kk - BufferLength + 2):kk + 1)); % dPI-102 [mbar] 
                    (measPlantArray(9,(kk - BufferLength + 2):kk + 1)); % dPI-103 [mbar]
...
for well = 1:3
    ...
    subplot(3,1,well)
        ...
        hold on 
        stairs(tgrid(2:end), yEstArray(well,:),'ko','Linewidth',1.5)
