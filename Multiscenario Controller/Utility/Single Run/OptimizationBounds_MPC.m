function [lbx,lbz,lbu,ubx,ubz,ubu,lbp,ubp] = OptimizationBounds_MPC(par)
...
%delta pressure - along erosion chamber [mbar]
prob_lb = 0.00001*ones(n_w,1);
prob_ub = 0.99999*ones(n_w,1);
...
%% 
lbx = vertcat(d_lb);
lbz = vertcat(m_o_lb,w_ro_lb,w_pr_lb,p_rh_lb,p_bh_lb,dp_lb,dD_lb,prob_lb);
lbu = vertcat(vo_lb,Ppump_lb);
lbp = vertcat(res_theta_lb,val_theta_lb);

ubx = vertcat(d_ub);
ubz = vertcat(m_o_ub,w_ro_ub,w_pr_ub,p_rh_ub,p_bh_ub,dp_ub,dD_ub,prob_ub);
