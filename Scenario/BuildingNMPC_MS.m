function solver = BuildingNMPC_MS(diff,alg,x_var,z_var,p_var,par,nmpcPar)
...
for l = 1:S
    ...
    % initial state
    x_prev = MX.sym(['X0_',num2str(l)],nmpcPar.nx);
    w = {w{:},x_prev}; 

    % initial input
    uk = MX.sym(['uk_init_',num2str(l)],nmpcPar.nu);
    w = {w{:}, uk}; 
    ...
    for k = 1:nmpcPar.np
        ...
        uk = MX.sym(['uk_',num2str(k),'_' num2str(l)],nmpcPar.nu);  
        w = {w{:}, uk};
        ...
        s = MX.sym(['s_',num2str(k),'_' num2str(l)],nmpcPar.nx);
        w = {w{:},s}; 
        ...
        for d = 1:3
            ...
            Xk = MX.sym(['Xk_' num2str(k),'_',num2str(d),'_' num2str(l)],nmpcPar.nx);
            Zk = MX.sym(['Zk_' num2str(k),'_',num2str(d),'_' num2str(l)],nmpcPar.nz);
            w = {w{:}, Xk, Zk}; 
        ...
        x_prev = MX.sym(['x_init_',num2str(k),'_' num2str(l)],nmpcPar.nx); 
        w = {w{:}, x_prev}; 
